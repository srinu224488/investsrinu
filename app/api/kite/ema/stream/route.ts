...
import { emaConfirmLatest } from "@/lib/ema-strategy";
import { kiteEmaFromOhlc } from "@/lib/kite-ema-ohlc";
...
/**
 * GET /api/kite/ema/stream — SSE: LTP merged into last candle; EMA confirm latest.
 */
export async function GET(req: NextRequest) {
  ...
  const length = Math.min(
    500,
    Math.max(1, Math.floor(Number(sp.get("length") ?? 21) || 21)),
  );
  ...
  let baseLatest: ReturnType<typeof emaConfirmLatest>;
  ...
  try {
    ...
    const result = await kiteEmaFromOhlc(accessToken, {
      instrumentToken,
      interval,
      from: displayFrom,
      to: displayTo,
    ...
    baseLatest = emaConfirmLatest(closesFull, strategy);
    ...
    start(controller) {
      ...
      ws.on("message", (raw) => {
        ...
        for (const t of ticks) {
          ...
          const latest = emaConfirmLatest(live, strategy);
          send({
            type: "tick",
            ltp,
            latest,

...
/**
 * GET /api/kite/ema — EMA(close) with consecutive-bar confirmation on Kite OHLC.
 *
 * Query: `symbol`, `from`, `to`, `interval` (default `day`),
 * `length` (EMA period, default 21), `confirm_bars` (default 1).
 */
export async function GET(req: NextRequest) {
  ...
  const length = Math.min(
    500,
    Math.max(1, Math.floor(Number(lengthRaw ?? 21) || 21)),
  );
  ...
  try {
    ...
    const message = e instanceof Error ? e.message : "ema_failed";

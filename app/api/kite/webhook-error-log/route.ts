...
import { listWebhookErrorLogPage } from "@/lib/webhook-error-log";
...
/**
 * GET /api/kite/webhook-error-log — persisted TradingView webhook failures for the UI.
 *
 * Auth: Kite session cookie, or `Authorization: Bearer <OPEN_ORDER_HISTORY_READ_SECRET>` (same as orderflow).
 */
export async function GET(req: NextRequest) {
  ...
  const page = await listWebhookErrorLogPage({ limit, offset });

...
import { maCrossConfirmLatest } from "@/lib/ma-cross-confirm-strategy";
...
/**
 * GET /api/kite/ma-cross/stream — Server-Sent Events: live LTP from Kite WebSocket
 * (https://www.kite.trade/docs/connect/v3/websocket/) merged into the last candle close,
 * then `maCrossConfirmLatest` for the selected interval + strategy params.
 *
 * Query: `symbol`, `interval`, `length`, `confirm_bars`; optional `from`, `to` (IST) for
 * initial historical load (defaults to a window sized by interval).
 *
 * Auth: Kite session cookie or server `KITE_ACCESS_TOKEN` / Mongo token (same as /api/kite/ma-cross).
 */
export async function GET(req: NextRequest) {
  ...
  const length = Math.min(
    500,
    Math.max(1, Math.floor(Number(sp.get("length") ?? 9) || 9)),
  );
  const confirmBars = Math.min(
    100,
    Math.max(1, Math.floor(Number(sp.get("confirm_bars") ?? 1) || 1)),
  );
  ...
  const strategy = { length, confirmBars };

  let instrumentToken: number;
  let closesFull: number[];
  let baseLatest: ReturnType<typeof maCrossConfirmLatest>;
    ...
    start(controller) {
      ...
      ws.on("message", (raw) => {
        ...
        for (const t of ticks) {
          ...
          const live = closesFull.slice();
          live[live.length - 1] = ltp;
          const latest = maCrossConfirmLatest(live, strategy);
          send({
            type: "tick",
            ltp,
            latest,
